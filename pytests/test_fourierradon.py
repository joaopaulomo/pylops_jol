...
def test_FourierRadon2D(par):
    ...
    if backend == "numpy" or (backend == "cupy" and par["engine"] == "numba"):
        dt, dh = 0.005, 1
        t = npp.arange(par["nt"]) * dt
        h = np.arange(par["nhx"]) * dh
        px = np.linspace(0, par["pxmax"], par["npx"])
        nfft = int(2 ** npp.ceil(npp.log2(par["nt"])))
        ...
        Rop = FourierRadon2D(
            ...
            kind=par["kind"],
            flims=[0, par["nt"] - 4],
            engine=par["engine"] if backend == "numpy" else "cuda",
            dtype="float64",
        )
        assert dottest(
            Rop,
